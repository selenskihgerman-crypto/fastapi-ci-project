...
def decrypt(encryption: str) -> str:
    if encryption.find('..') >= 0:
        dot_pos: int = encryption.find('..')
        if dot_pos == 0:
            my_description = encryption[:dot_pos] + '' +encryption[dot_pos + 2:]
        else:
            my_description = encryption[:dot_pos - 1] + '' +encryption[dot_pos + 2:]
        return decrypt(my_description)

    elif encryption.find('.') >= 0:
        dot_pos = encryption.find('.')
        my_description = encryption[:dot_pos] + '' + encryption[dot_pos + 1:]
        return decrypt(my_description)
    else:
        return encryption
